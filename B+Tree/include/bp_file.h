...
// Ο τύπος των κόμβων του B+ δέντρου
typedef enum{
    BPLUS_LEAF,
    BPLUS_INDEX
} BPLUS_NODE_TYPE;

// Η επιστρεφόμενη τιμή από την προσπάθεια εισαγωγής μιας εγγραφής	
typedef enum {
    BP_OK,
    BP_LEAF_FULL,
    BP_LEAF_HAS_DUPLICATE
} BP_INSERT_STATUS;

typedef struct {
    int root_block_id;      // Το id της ρίζας
    int last_block_id;      // Το id του τελευταίου block
    int max_index_count;        // Ο μέγιστος αριθμός κλειδιών που μπορεί να έχει ένας κόμβος ευρετηρίου
    int max_records_count;      // Ο μέγιστος αριθμός εγγραφών που μπορεί να έχει ένα φύλλο
} BPLUS_INFO;
